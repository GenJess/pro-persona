...
import { Upload, Users, ArrowRight, Sparkles, MessageSquare, Globe, Zap, Brain, Coffee, Search, Briefcase, Star, Mic, FileText } from "lucide-react";
...
const Index = () => {
  ...
  const useCases = [
    ...
    },
    {
      ...
      description: "Want to know what someone's building? Ask their agent. It's like a coffee chat — async, instant, and always available.",
    },
  ];

  // Showcase featured agents
  const showcaseAgents = [
    { name: "Steve Jobs", role: "Co-founder, Apple", initials: "SJ", color: "bg-foreground" },
    { name: "Jony Ive", role: "Former CDO, Apple", initials: "JI", color: "bg-primary" },
    { name: "Lisa Su", role: "CEO, AMD", initials: "LS", color: "bg-destructive" },
    { name: "Jensen Huang", role: "CEO, NVIDIA", initials: "JH", color: "bg-primary/80" },
  ];
      ...
      <section className="relative w-full flex flex-col items-center pt-20 md:pt-32 pb-24 md:pb-36 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-4xl mx-auto px-4">
          ...
          <div className="mt-12 flex gap-4 justify-center flex-wrap opacity-0 animate-fade-in stagger-3">
            <Button size="lg" asChild className="h-13 px-8 text-base font-semibold shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 transition-all">
              <Link to="/signup">
                Build Your Agent
                <ArrowRight className="ml-2 h-5 w-5" />
              </Link>
            </Button>
            <Button size="lg" variant="outline" asChild className="h-13 px-8 text-base font-semibold hover:bg-accent/50 transition-all">
              <Link to="/personas">Discover People</Link>
            </Button>
          </div>

          {/* Agent Showcase Avatars */}
          <div className="mt-14 opacity-0 animate-fade-in stagger-4">
            <p className="text-sm text-muted-foreground mb-4">Featured agents on the platform</p>
            <div className="flex items-center justify-center gap-3 flex-wrap">
              {showcaseAgents.map((agent) => (
                <Link 
                  to="/personas" 
                  key={agent.name}
                  className="flex items-center gap-2.5 px-4 py-2.5 rounded-full border border-border/50 bg-card hover:border-primary/30 hover:shadow-md transition-all cursor-pointer"
                >
                  <Avatar className="h-8 w-8">
                    <AvatarFallback className={`${agent.color} text-primary-foreground text-xs font-bold`}>
                      {agent.initials}
                    </AvatarFallback>
                  </Avatar>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-primary/20 p-12 md:p-20 text-center">
          ...
          <div className="relative z-10">
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              <Button size="lg" asChild className="h-13 px-10 text-base font-semibold shadow-lg shadow-primary/20">
                <Link to="/signup">
                  Create Your Agent
                  <ArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </Button>
              <Button size="lg" variant="outline" asChild className="h-13 px-10 text-base font-semibold">
                <Link to="/personas">
                  Explore Agents
                </Link>
              </Button>
